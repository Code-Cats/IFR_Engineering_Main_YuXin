...
void Take_Bullet_Task(void)
{
	...
	if(GetWorkState()==NORMAL_STATE)	//取弹升降给DOWN-MID，前伸出发-夹紧一套给DOWN-MID-->DOWN-DOWN;舵机旋转给DOWN-MID-->DOWN-UP
	{
		if(RC_Ctl.rc.switch_left==RC_SWITCH_DOWN)
		{
			...
			{
				ViceControlData.servo[0]=!ViceControlData.servo[0];
			}
...
#define BULLETLIFT_DISTANCE_GRIPBULLET	500	//夹弹药箱时夹持升降高度
